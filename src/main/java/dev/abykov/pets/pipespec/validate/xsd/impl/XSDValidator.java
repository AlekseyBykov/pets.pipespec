package dev.abykov.pets.pipespec.validate.xsd.impl;

import dev.abykov.pets.pipespec.validate.xsd.IXSDValidator;
...
public class XSDValidator implements IXSDValidator {
    ...
    private void prepareErrorMessages(XSDErrorHandler errorHandler, File scheme) {
        ...
        exceptions.stream()
                .map(e -> String.format(
                        "PipeSpec schema %s does not conform to XSD, line: %s, col: %s. %s",
